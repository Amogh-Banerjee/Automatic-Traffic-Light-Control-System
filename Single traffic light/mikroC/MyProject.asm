...
_setRed:
;MyProject.c,13 :: 		void setRed(){ // This function enables the red signal when requested by the user and then the system continues to operate normally
...
_setOrange:
;MyProject.c,29 :: 		void setOrange(){ // This function enables the orange signal when requested by the user and then the system continues to operate normally
...
_setGreen:
;MyProject.c,45 :: 		void setGreen(){ // This function enables the green signal when requested by the user and then the system continues to operate normally
...
;MyProject.c,63 :: 		color = UART1_READ(); // Read the data received via UART
	LCALL _UART1_Read+0
...
L_setDisplay10:
;MyProject.c,67 :: 		setRed(); // If the received character is 'r' then enable the red light
	LCALL _setRed+0
;MyProject.c,68 :: 		break;
	SJMP L_setDisplay9
;MyProject.c,69 :: 		case 'o':
L_setDisplay11:
;MyProject.c,70 :: 		setOrange(); // If the received character is 'o' then enable the orange light
	LCALL _setOrange+0
;MyProject.c,71 :: 		break;
	SJMP L_setDisplay9
;MyProject.c,72 :: 		case 'g':
L_setDisplay12:
;MyProject.c,73 :: 		setGreen(); // If the received character is 'g' then enable the green light
	LCALL _setGreen+0
...
;MyProject.c,84 :: 		P1 = Dec2Bcd(count); // Converting the count to BCD and displaying it on the 7-segment display
	MOV FARG_Dec2Bcd_decnum+0, FARG_countAndCheck_count+0
	LCALL _Dec2Bcd+0
	MOV P1+0, 0
;MyProject.c,85 :: 		delay_ms(1000); // Generating a delay of 1 second
	MOV R5, 8
	MOV R6, 1
	MOV R7, 245
	DJNZ R7, 
	...
	DJNZ R5, 
	NOP
;MyProject.c,86 :: 		count--; // Decrementing the count by 1
	CLR C
	MOV A, FARG_countAndCheck_count+0
	SUBB A, #1
...
;MyProject.c,88 :: 		if (UART1_DATA_READY()) // Checking whether some data is received at the serial port
	LCALL _UART1_Data_Ready+0
	MOV A, R0
	JZ L_countAndCheck16
;MyProject.c,90 :: 		break; // Exit the loop if some data is received
...
;MyProject.c,99 :: 		P2.B2 = 0;
	CLR P2+0
;MyProject.c,100 :: 		setDisplay(); // If some data is received at the serial port then disable all the lights and call the setDisplay() function
	LCALL _setDisplay+0
;MyProject.c,101 :: 		}
L_countAndCheck17:
;MyProject.c,102 :: 		}
...
;MyProject.c,107 :: 		P1 = 0;
	MOV P1+0, #0
;MyProject.c,108 :: 		P2.B0 = 0;
	CLR P2+0
;MyProject.c,109 :: 		P2.B1 = 0;
	CLR P2+0
;MyProject.c,110 :: 		P2.B2 = 0;
	CLR P2+0
;MyProject.c,112 :: 		UART1_INIT(9600); // Initialising UART with a Baud rate of 9600
	MOV TH1+0, #253
	MOV TL1+0, #253
	LCALL _UART1_Init+0
;MyProject.c,113 :: 		PCON.SMOD = 0;
	CLR C
	MOV A, PCON+0
	MOV #224, C
	MOV PCON+0, A
;MyProject.c,114 :: 		UART1_WRITE_TEXT("System is ready. \r\n "); // Sending some messages to the virtual terminal via UART
	MOV FARG_UART1_Write_Text_uart_text+0, #?lstr2_MyProject+0
	LCALL _UART1_Write_Text+0
;MyProject.c,115 :: 		UART1_WRITE_TEXT("R RED, ");
	MOV FARG_UART1_Write_Text_uart_text+0, #?lstr3_MyProject+0
	LCALL _UART1_Write_Text+0
;MyProject.c,116 :: 		UART1_WRITE_TEXT("O ORANGE, ");
	MOV FARG_UART1_Write_Text_uart_text+0, #?lstr4_MyProject+0
	LCALL _UART1_Write_Text+0
;MyProject.c,117 :: 		UART1_WRITE_TEXT("G GREEN\r\n");
	MOV FARG_UART1_Write_Text_uart_text+0, #?lstr5_MyProject+0
	LCALL _UART1_Write_Text+0
;MyProject.c,119 :: 		while (1) // This loop is for the automatic operation of the system
L_main18:
;MyProject.c,121 :: 		P2.B0 = 0;
	CLR P2+0
;MyProject.c,122 :: 		P2.B2 = 1; // Disabling the red light and enabling the green light
	SETB P2+0
;MyProject.c,123 :: 		countAndCheck(10); // The signal stays for 10 seconds and the timing is displayed on the 7-segment display
	MOV FARG_countAndCheck_count+0, #10
	MOV FARG_countAndCheck_count+1, #0
	LCALL _countAndCheck+0
;MyProject.c,124 :: 		P2.B2 = 0;
	CLR P2+0
;MyProject.c,125 :: 		P2.B1 = 1; // Disabling the green light and enabling the orange light
	SETB P2+0
;MyProject.c,126 :: 		countAndCheck(10);
	MOV FARG_countAndCheck_count+0, #10
	MOV FARG_countAndCheck_count+1, #0
	LCALL _countAndCheck+0
;MyProject.c,127 :: 		P2.B1 = 0;
	CLR P2+0
;MyProject.c,128 :: 		P2.B0 = 1; // Disabling the orange light and enabling the red light
	SETB P2+0
;MyProject.c,129 :: 		countAndCheck(20);
